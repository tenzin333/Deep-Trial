...
- **Semantic Embeddings** — Every page is converted into a 1536-dimension vector that captures its *meaning*, not just keywords
...
FastAPI Backend
        ...
        ├─→ Embedder (OpenAI text-embedding-3-small → 1536-d vector)
...
PostgreSQL 16 + pgvector
        ...
        ├── nodes (url, title, summary, keywords, embedding VECTOR(1536))
...
| Vector Store | pgvector | VECTOR(1536) columns + HNSW indexing |
| Text Extraction | trafilatura | HTML to clean article text |
| Embeddings | OpenAI text-embedding-3-small | 1536-d semantic vectors |
...
│   │   │   ├── user.py             # User table
│   │   │   ├── node.py             # Node table + Vector(1536)
...
4. Backend fetches page → extracts clean text
         ↓
5. Text → OpenAI → 1536-d embedding vector
         ↓
